...
function parseOpenMeteo(data, rainThreshold = 0.1) {
    ...
    return {
        raining: precipitation >= rainThreshold || number(current.rain, 'rain') >= rainThreshold,
        precipitation,
...
function parseBrightSky(data, rainThreshold = 0.1) {
    ...
    return {
        raining: precipitation >= rainThreshold || (current.condition && current.condition !== 'dry'),
        precipitation,
...
async function getJson(url, timeoutMs = 10000) {
    ...
    try {
        const response = await fetch(url, { signal: controller.signal, headers: { 'User-Agent': 'ioBroker.mowtime/0.1' } });
...
async function fetchWeather(provider, latitude, longitude, rainThreshold, fetchJson = getJson) {
    ...
    if (provider === 'openmeteo') {
        const url = `https://api.open-meteo.com/v1/forecast?latitude=${encodeURIComponent(lat)}&longitude=${encodeURIComponent(lon)}&current=temperature_2m,precipitation,rain,wind_speed_10m&timezone=auto`;
