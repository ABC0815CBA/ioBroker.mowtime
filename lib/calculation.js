...
function calendarDayMinutes(date, ...calendars) {
    const mondayIndex = (date.getDay() + 6) % 7;
    return calendars.map(parseCalendar).reduce((sum, calendar) => {
        const minutes = Number(calendar[mondayIndex]?.[1]);
        return sum + (Number.isFinite(minutes) && minutes > 0 ? minutes : 0);
    }, 0);
}
...
module.exports = { SOIL_FACTORS, SHADE_FACTORS, calendarMinutes, calendarDayMinutes, targetMinutes, distributeZones, decide };
