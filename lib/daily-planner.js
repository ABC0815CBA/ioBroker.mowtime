...
function settleDay(targetByZone, actualMinutes) {
    const targetTotal = targetByZone.reduce((sum, value) => sum + Math.max(0, Number(value) || 0), 0);
    const actualByZone = Array.isArray(actualMinutes)
        ? targetByZone.map((_, zone) => Math.max(0, Number(actualMinutes[zone]) || 0))
        : targetByZone.map(value => targetTotal > 0 ? Math.max(0, actualMinutes) * Math.max(0, value) / targetTotal : 0);
